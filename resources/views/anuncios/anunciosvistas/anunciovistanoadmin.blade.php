        ...
        <div class="shadow-lg card" style="background-color: #f4f4f4; width: 50rem">
            ...
            @endforeach
              ...
              <div class="container">
                ...
                @if($con==0)
                <button style="border:0px" type="button" class="btn btn-outline-secondary" wire:click="guardar({{$anuncio->ID_ANUNCIOS}})" style="margin: 10px"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-bookmark-plus-fill" viewBox="0 0 16 16">
      ...
      <br>
        <div class="shadow-lg card" style="background-color: #c4c4c4; width: 50rem">
            ...
            @php
                ...
                $vid = 0;
                $pdf = 0;
                if (strpos($filtro->MULTIMEDIA, '.jpg' ) !== false || strpos($filtro->MULTIMEDIA, '.png' ) !== false || strpos($filtro->MULTIMEDIA, '.jpeg' ) !== false) 
                 { $foo=1; }
                elseif(strpos($filtro->MULTIMEDIA, '.mp4' ) !== false || strpos($filtro->MULTIMEDIA, '.mpeg' ) !== false)
                 {$vid=1;}
                 elseif(strpos($filtro->MULTIMEDIA, '.pdf' ) !== false)
                 {$pdf=1;}
                 
            @endphp

            @if($foo==1)
              <img src="imagen/anuncios/{{$filtro->MULTIMEDIA}}" height="500" weight="500" class="card-img-top" alt="...">
            @endif
            @if($vid==1)
            <video height="500" weight="500" class="card-img-top" alt="..." controls>
              <source src="imagen/videos/{{$filtro->MULTIMEDIA}}"  type="video/mp4">
            </video>
            @endif
            @if($pdf==1)
             <iframe style="width: 49rem; text-align:center" width="400" height="400" src="/imagen/pdfs/{{$filtro->MULTIMEDIA}}" frameborder="0"></iframe>
            @endif
            
            <div class="card-body">
              @foreach($usuario_publicacion as $usu_publicacion)
              @if($filtro->ID_USUARIO == $usu_publicacion->id)
              <h5 class="card-title">{{$usu_publicacion->usuario}}</h5>
              @endif
              @endforeach
              
              @foreach($rol_publicado as $rol_publi)
              @if($filtro->ID_USUARIO == $rol_publi->ID_USUARIO)
              @if($rol_publi->ID_ROL==1)
              <p class="card-text" style="font-size:12px">Superusuario</p>
              @elseif($rol_publi->ID_ROL==2)
              <p class="card-text" style="font-size:12px">Administrador</p>
              ...
              @endif
              @endforeach
              <p class="card-text">{{$filtro->TEXTO_PUBLICACION}}</p>
              
              
              @if($filtro->CALIDAD_ANUNCIO==1)
              <div class="alert alert-success d-flex align-items-center rounded-pill" role="alert">
                <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Info:"><use xlink:href="#info-fill"/></svg>
                <div>
                  Informativo
              ...
              @elseif($filtro->CALIDAD_ANUNCIO==2)
              <div class="alert alert-warning d-flex align-items-center rounded-pill" role="alert">
                <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Warning:"><use xlink:href="#exclamation-triangle-fill"/></svg>
                <div>
                  Importante
              ...
              @foreach($vistoss as $visto)
              @if($filtro->ID_ANUNCIOS == $visto->ID_ANUNCIO && $visto->ID_USUARIO != $vistas_totales_id)
              @php
              //$vistas_totales_id representa al usuario logeado en ese momento
              $contador_vistas+=1;
              @endphp
              ...
              <div class="container">
                <div class="row">
                  <div class="input-group mb-3">
                    ...
                    @foreach($me_gusta as $gustaa)
                    @if($filtro->ID_ANUNCIOS == $gustaa->ID_PUBLICACION && $gustaa->ID_USUARIO == auth()->user()->id)
                    @php
                    $can = 1;
                    @endphp
                    ...
                    @if($can == 1)
                    <button style="border:0px" type="button" class="btn btn-primary" wire:click="eliminarmegusta({{$filtro->ID_ANUNCIOS}})" style="margin: 10px"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-hand-thumbs-up-fill" viewBox="0 0 16 16">
                      <path d="M6.956 1.745C7.021.81 7.908.087 8.864.325l.261.066c.463.116.874.456 1.012.965.22.816.533 2.511.062 4.51a9.84 9.84 0 0 1 .443-.051c.713-.065 1.669-.072 2.516.21.518.173.994.681 1.2 1.273.184.532.16 1.162-.234 1.733.058.119.103.242.138.363.077.27.113.567.113.856 0 .289-.036.586-.113.856-.039.135-.09.273-.16.404.169.387.107.819-.003 1.148a3.163 3.163 0 0 1-.488.901c.054.152.076.312.076.465 0 .305-.089.625-.253.912C13.1 15.522 12.437 16 11.5 16H8c-.605 0-1.07-.081-1.466-.218a4.82 4.82 0 0 1-.97-.484l-.048-.03c-.504-.307-.999-.609-2.068-.722C2.682 14.464 2 13.846 2 13V9c0-.85.685-1.432 1.357-1.615.849-.232 1.574-.787 2.132-1.41.56-.627.914-1.28 1.039-1.639.199-.575.356-1.539.428-2.59z"/>
                    </svg> Me gusta </button>
                    @else
                    <button style="border:0px" type="button" class="btn btn-outline-primary" wire:click="insertar_like({{$filtro->ID_ANUNCIOS}})" style="margin: 10px"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-hand-thumbs-up-fill" viewBox="0 0 16 16">
                      <path d="M6.956 1.745C7.021.81 7.908.087 8.864.325l.261.066c.463.116.874.456 1.012.965.22.816.533 2.511.062 4.51a9.84 9.84 0 0 1 .443-.051c.713-.065 1.669-.072 2.516.21.518.173.994.681 1.2 1.273.184.532.16 1.162-.234 1.733.058.119.103.242.138.363.077.27.113.567.113.856 0 .289-.036.586-.113.856-.039.135-.09.273-.16.404.169.387.107.819-.003 1.148a3.163 3.163 0 0 1-.488.901c.054.152.076.312.076.465 0 .305-.089.625-.253.912C13.1 15.522 12.437 16 11.5 16H8c-.605 0-1.07-.081-1.466-.218a4.82 4.82 0 0 1-.97-.484l-.048-.03c-.504-.307-.999-.609-2.068-.722C2.682 14.464 2 13.846 2 13V9c0-.85.685-1.432 1.357-1.615.849-.232 1.574-.787 2.132-1.41.56-.627.914-1.28 1.039-1.639.199-.575.356-1.539.428-2.59z"/>
                    </svg> Me gusta </button>
                    @endif
                
                <button style="border:0px" type="button" class="btn btn-outline-secondary" wire:click='comentario({{$filtro->ID_ANUNCIOS}})' style="margin: 10px" id="Crear"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-hand-thumbs-up-fill" viewBox="0 0 16 16">
                    <path d="M0 2a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H4.414a1 1 0 0 0-.707.293L.854 15.146A.5.5 0 0 1 0 14.793V2zm3.5 1a.5.5 0 0 0 0 1h9a.5.5 0 0 0 0-1h-9zm0 2.5a.5.5 0 0 0 0 1h9a.5.5 0 0 0 0-1h-9zm0 2.5a.5.5 0 0 0 0 1h5a.5.5 0 0 0 0-1h-5z"/>
                    </svg> Comentarios</button>
                @include('anuncios.comentariosvista.com')
                @php
                ...
                @endphp
                @foreach($guardar as $guarda)
                @if($filtro->ID_ANUNCIOS == $guarda->ID_ANUNCIO && $guarda->ID_USUARIO == $usuario_id)
                @php
                $con=1;
                @endphp
                @endif
                ...
                @if($con==0)
                <button style="border:0px" type="button" class="btn btn-outline-secondary" wire:click="guardar({{$filtro->ID_ANUNCIOS}})" style="margin: 10px"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-bookmark-plus-fill" viewBox="0 0 16 16">
                 <path fill-rule="evenodd" d="M2 15.5V2a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v13.5a.5.5 0 0 1-.74.439L8 13.069l-5.26 2.87A.5.5 0 0 1 2 15.5zm6.5-11a.5.5 0 0 0-1 0V6H6a.5.5 0 0 0 0 1h1.5v1.5a.5.5 0 0 0 1 0V7H10a.5.5 0 0 0 0-1H8.5V4.5z"/>
                </svg> Guardar</button>
                @endif
               </div>
